...
CREATE TABLE IF NOT EXISTS MD_STOCKYARD (
    ...
    YARD_NO         VARCHAR(10) UNIQUE NOT NULL,
    X_COORDINATE    INT NOT NULL,
    Y_COORDINATE    INT NOT NULL,
    DESCRIPTION     VARCHAR(256),
    YARD_TYPE       CHAR(1) NOT NULL,      -- I=Internal, E=External, S=Saw, A=SwapOut/Ausgang, N=SwapIn/Eingang, L=Loading
...
CREATE TABLE IF NOT EXISTS MD_INGOTTYPE (
    ...
    SAW_TO_SWAPOUT  BOOLEAN DEFAULT FALSE,   -- Automatisch von Säge zum Ausgang (für Stapler)
    MIN_LENGTH      INT,
    MAX_LENGTH      INT,
    MIN_WIDTH       INT,
    MAX_WIDTH       INT,
...
-- Shipment (Lieferschein) - Bewegungsdaten
CREATE TABLE IF NOT EXISTS TD_SHIPMENT (
    ID              BIGINT PRIMARY KEY,
    SERIAL          BIGINT NOT NULL DEFAULT 1,
...
-- ShipmentLine (Lieferschein-Position) - Bewegungsdaten
CREATE TABLE IF NOT EXISTS TD_SHIPMENTLINE (
    ID              BIGINT PRIMARY KEY,
    SERIAL          BIGINT NOT NULL DEFAULT 1,
    SHIPMENT_ID     BIGINT NOT NULL REFERENCES TD_SHIPMENT(ID),
    SHIPMENT_POS    INT NOT NULL,
    INGOT_NO        VARCHAR(20),
    INGOT_ID        BIGINT,
    INGOT_COMMENT   VARCHAR(200),
    WEIGHT          INT,
    LENGTH          INT,
    THICKNESS       INT,
    WIDTH           INT,
    ...
    SAP_PRODUCT_NO  VARCHAR(50),
    CALLOFF_NO      VARCHAR(50),
    ORDER_POS       VARCHAR(50)
);

CREATE INDEX IF NOT EXISTS IDX_SHIPMENT_NO ON TD_SHIPMENT(SHIPMENT_NO);
CREATE INDEX IF NOT EXISTS IDX_SHIPMENT_DELIVERED ON TD_SHIPMENT(DELIVERED);
CREATE INDEX IF NOT EXISTS IDX_SHIPMENTLINE_SHIPMENT ON TD_SHIPMENTLINE(SHIPMENT_ID);

-- Calloff (Abruf/Kundenbestellung) - Bewegungsdaten
CREATE TABLE IF NOT EXISTS TD_CALLOFF (
    ID                  BIGINT PRIMARY KEY,
    SERIAL              BIGINT NOT NULL DEFAULT 1,
    TABLESERIAL         BIGINT NOT NULL DEFAULT 1,
    CALLOFF_NUMBER      VARCHAR(50) NOT NULL,
    ...
    PRODUCT_ID          BIGINT REFERENCES MD_PRODUCT(ID),
    AMOUNT_REQUESTED    INT DEFAULT 0,
    ...
    RECEIVED            TIMESTAMP,
    NOTES               VARCHAR(500),
    CONSTRAINT UK_CALLOFF_NUMBER UNIQUE (CALLOFF_NUMBER)
);

CREATE INDEX IF NOT EXISTS IDX_CALLOFF_APPROVED ON TD_CALLOFF(APPROVED);
CREATE INDEX IF NOT EXISTS IDX_CALLOFF_COMPLETED ON TD_CALLOFF(COMPLETED);
CREATE INDEX IF NOT EXISTS IDX_CALLOFF_PRODUCT ON TD_CALLOFF(PRODUCT_ID);
